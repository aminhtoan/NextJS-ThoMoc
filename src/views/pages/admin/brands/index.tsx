...
import CreateBrands from './components/brand/CreateBrand'
import DeleteBrands from './components/brand/DeleteBrand'
import UpdateBrands from './components/brand/UpdateBrand'
...
const BrandsPage: NextPage<TProps> = () => {
  ...
  const canCreate = ability?.can(METHOD_MAP.POST, MODULES.BRAND)
  const canUpdate = ability?.can(METHOD_MAP.PUT, MODULES.BRAND)
  const canDelete = ability?.can(METHOD_MAP.DELETE, MODULES.BRAND)
  ...
  const handleRefreshTable = useCallback(async () => {
    try {
      setLoading(true)
      const response = await GetBrand({ page: 1, limit: 100 })

      const tableData: UserTableRow[] = response.data.data.map((item: any) => ({
        id: item.id,
        name: item.brandTranslations[0]?.name || item.name || 'N/A', // Assuming translations is an array and we take the first one
        logo: item.logo,
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <CreateBrands open={isOpenCreateUser} onClose={() => setIsOpenCreateUser(false)} onCreated={handleRefreshTable} />
